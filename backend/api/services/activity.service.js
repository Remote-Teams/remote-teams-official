...
function searchActivities(searchCriteria) {
    ...
    let projectId = query.projectId, entityType = query.type;
    //console.log(projectId, activityType);
    const options = {};
    options.skip = pageSize * (pageNo - 1);
    options.limit = pageSize;
    
       var data = await activityModel.search({$and:[{"projectId":projectId},{"entityType":entityType}]})
       resolve(data)
