...
function init(router) {
  router.route('/meetings')
    .get(getAllMeetings)
    .post(addMeeting);
  router.route('/meetings/count/:id')
...
function getTodayMeetings(req, res, next) {
  var searchCriteria = {
    ...
    query:{
      "$and":[
        { "meetingDate" : { "$gte":moment().startOf('day').toISOString() } },
        { "meetingDate" : { "$lte":moment().endOf('day').toISOString() } }
      ]
    }
  }
  meetingService.searchMeetings(searchCriteria).then((data) => {
...
function getAllMeetingsCount(req,res,next) {
  ...
  meetingService.getAllMeetingsCount(id).then((data) => {
      if(data == undefined){
        return next(errorMethods.sendBadRequest(errorCode.MEETING_NOT_EXIST));
      }
      res.send({ 'count': data });
