...
//var featurePrices = require('../../config/livefeaturePrice.json');
var featurePrices = require('../../config/testfeaturePrice.json');
var paidFeatures = require('../../config/paidFeatures.json');
...
router.post('/featureplans', async function (req, res, next) {
        try{
        let reqcurrency = req.body.currency
        features = featurePrices.featurePrices
        planData = features.filter(data => data.currency == reqcurrency)
        res.json(planData)
        }catch(err){reject(err)}
        });
...
router.post('/paidFeatures', function (req, res, next) {
    try{
    var plan = req.body.plan
    var planData = _.find(paidFeatures.featurePlans, { name:plan })
    planData === undefined 
    ? next(errorMethods.sendBadRequest(errorCode.PLAN_DOES_NOT_EXIST))
    : res.json(planData)
    }catch(err){next(errorMethods.sendServerError(errorCode.INTERNAL_SERVER_ERROR));}
});

router.post('/findFeature', function(req,res,next){
    try{
    var feature = req.body.feature
    var arkPlan = _.find(paidFeatures.featurePlans, { "name":"Ark" })
    var metaversePlan = _.find(paidFeatures.featurePlans, { "name":"Metaverse" })
    let result = _.find(arkPlan.features, function(n) {
        if (n == feature) {
            return true;
        }
    });  
    if (result===undefined){
        newResult = _.find(metaversePlan.features, function(n) {
            if (n == feature) {
                return true;
            }
        }) 
        newResult === undefined
        ? next(errorMethods.sendBadRequest(errorCode.FEATURE_DOES_NOT_EXIST))
        : res.json("METAVERSE")
    }else{res.json("ARK")}
    }catch(err){next(errorMethods.sendServerError(errorCode.INTERNAL_SERVER_ERROR));}
})
