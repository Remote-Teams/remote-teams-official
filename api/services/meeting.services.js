...
function addMeeting(meetingData) {
    let attendeesUser = meetingData.attendees;
    return new Promise((resolve, reject) => {
        ...
        meetingModel.create(meetingData).then((data) => {
            // meetingModel.getById(data.id).then((meetingResult) => {
                ...
                notificationServices.addNotification(payload, user.workspaceId)
                notificationClient.notifyMeeting(NotificationType.MEETING_CREATED, data, user.workspaceId, user.userId,attendeesUser);
                resolve(data);
            // }).catch((err) => {
                // reject(err);
            // })
...
function getAllMeetingsCount(id) {
    return new Promise((resolve, reject) => {
        meetingModel.countDocuments(id).then((data) => {
            resolve(data);
        }).catch((err) => {
            reject(err);
        })
