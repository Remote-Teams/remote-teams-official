...
import {
  createScheduleInBookCalender,
  createBulkScheduleInBookCalender,
} from "./../../../store/actions/bookTheirCalenderAction";
...
import { SET_ASSIGNED_MEMBERS_LEFT_ARRAY } from "./../../../store/types";
...
function BookCalenderAddScheduleOnCellClick({
  bookCalenderModel,
  onCloseModal,
  bookOnCellClickData,
  memberId,
...
===================================*/
  ...
  const callBackAddSchedule = (status, data) => {
    if (status === 200) {
      onCloseModal();
      // const array = updateTaskMemberLeftArray;
      // const index = array.indexOf(data.assignedTo);
      // if (index > -1) {
  ...
  const handleSave = (e) => {
    ...
    if (
      ...
      dispatch(createScheduleInBookCalender(formData, callBackAddSchedule));
    } else {
      const { startDate, endDate } = values;
      ...
      dispatch(createBulkScheduleInBookCalender(formData, callBackAddSchedule));
    }
  };
    ...
    <div>
      <Modal
        ...
        classNames={{
          ...
          //modal: "customModal customModal--createSchedule",
...
export default BookCalenderAddScheduleOnCellClick;
