...
import { update_draft } from '../../../store/actions/emailAction';
...
export class CreateEmails extends Component {
    ...
    componentDidMount(){
        ...
        let data = {
            id:this.props.connectappEmail.all_drafts.threads.drafts[this.props.editIndex].id,
            from:payloads.find( data => data.name === "From" ) ? payloads.find( data => data.name === "From" ).value : "",
            to:payloads.find( data => data.name === "To" ) ? payloads.find( data => data.name === "To" ).value.split(",") : [],
    ...
    onSendHandler = e => {
        ...
        this.props.update_draft(formData, null , "send=true");
        this.onClose();
    }
    onFileSelector = e => {
    ...
    render() {
        return (
            <div>
                <WrapperComponent  { ...this.state } { ...this.localProps }>
