...
import { updateScrumById } from "./../../../store/actions/projectAction";
...
export default function AllProjectModifyScrum({ scrumData }) {
  ...
  useEffect(() => {
    if (!isEmpty(scrumData)) {
      let defaultMemeber = !isEmpty(scrumData.attendees)
        ? scrumData.attendees.map((user) => ({
            value: user._id,
      ...
      setValues({
        ...values,
        startDate: new Date(scrumData.fromDate),
        endDate: new Date(scrumData.toDate),
        fromTime: setHours(setMinutes(new Date(scrumData.fromTime), 0), 10),
        toTime: setHours(setMinutes(new Date(scrumData.toTime), 0), 11),
        title: scrumData.name,
        isStatusActiveMail: scrumData.emailNotify,
        isStatusActive: scrumData.daily_scrum,
  ...
  const handleSave = () => {
    ...
    if (!isEmpty(values.displayListSelected)) {
      values.displayListSelected.forEach((element) => {
        selectedMembers.push(element.value);
      });
    }
    let formData = scrumData;

    formData.name = values.title;
    formData.attendees = selectedMembers;
    ...
    formData.emailNotify = values.isStatusActiveMail;
    formData.daily_scrum = values.isStatusActive;

    dispatch(updateScrumById(formData, callBackUpdate));
  };
  ...
  return (
    <>
      <GrayButtonSmallFont text={"Edit"} onClick={onHandleModal} />
      <Modal
        open={open}
          ...
          <div className="row flex-nowrap mx-0 all-project-scrum-modal-row2 pt-3">
            ...
            <div className="shedule-toggle-2">
              ...
              <ToggleTimesheet
                ...
                defaultChecked={values.isStatusActiveMail}
              />
            </div>
          </div>
          <div className="pb-10">
