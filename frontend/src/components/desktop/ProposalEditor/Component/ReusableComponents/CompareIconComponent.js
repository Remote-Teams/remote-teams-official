...
import CompareIcon from "../../asset/compareIcon";
...
export class CompareIconComponent extends Component {
  constructor(props) {
    super(props);
    this.state = {
      selectedStack: "",
      ...
      <div
        ...
        className={
          this.state.selectedStack === this.props.stackClass
            ? "icon_selection_component_main_container_selected"
            : "icon_selection_component_main_container"
        }
        style={{
          backgroundColor: this.props.icon_background,
          position: "relative",
        }}
      >
        {this.state.selectedStack === this.props.stackClass ? (
          <IconSetting {...this.props} />
        ) : null}
        <i
          style={{ color: this.props.icon_color }}
          className={`fa ${this.props.icon_name} ${this.props.icon_size}`}
        ></i>
      </div>
    );
  }
...
export const IconSetting = (props) => {
  ...
  return (
    <>
      <div
        id={props.icon_modal_name}
        onClick={() => imageSettingHandler(true)}
        className="image_cogs"
      >
          ...
          <div id={props.icon_modal_name} className="modal_main_container">
            ...
            <div className="icon_setting_main_container">
              <div className="dispaly_selected_icon">
                <div
                  className="icon_selection_component_main_container"
                  style={{
                    backgroundColor: props.icon_background,
              ...
              <div className="icon_setting_size_row">
                {faSize.map((isize, index) => (
                  <div
                    key={index}
                    className={
                      props.icon_size === isize.value
                        ? "icons_size_button_selected"
                        : "icons_size_button"
                    }
                    onClick={props.onTemplateItemChange(
                      props.icon_size_name,
...
export const IconSelector = (props) => {
  console.log(CompareIcon.length);
  const [iconSelector, iconSelectorHandler] = useState(false);
  return (
    <>
      <div
        className="select_new_icon_container"
        onClick={() => iconSelectorHandler(true)}
      >
        Select New Icon
      </div>
      <Modal
        ...
        size={"xl"}
        centered
        onHide={() => iconSelectorHandler(false)}
      >
        <Modal.Body>
          <div id={props.icon_modal_name} className="modal_main_container">
            <div className="modal__close_icon_container">
              <div
                className="modal__close_icon"
                onClick={() => iconSelectorHandler(false)}
              >
                <i className="fa fa-times" aria-hidden="true"></i>
              </div>
            </div>
            ...
            <div className="modal__button_container">
              <button onClick={() => iconSelectorHandler(false)}>Save</button>
            </div>
          </div>
        </Modal.Body>
      </Modal>
