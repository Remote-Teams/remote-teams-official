...
export default class Header extends PureComponent {
  ...
  renderHeaderRows = (top, middle, bottom) => {
    ...
    let date = new Date();
    let todayDay = dateFns.format(date, "D").toString();
    let todayMonth = dateFns.format(date, "MMMM YYYY");
      ...
      if (currentBottom != currentDate.format(this.getFormat(bottom))) {
        ...
        if (bottom == "shorttime" || bottom == "fulltime") {
          ...
          result.bottom.push(
            <HeaderItem
              className={
                currentBottom === todayDay && todayMonth === currentTop
                  ? "todays-date"
                  : ""
              }
              key={i}
  ...
  doMouseMove = (e) => {
    this.props.onMouseMove(e, this.refs.Header);
  };
  ...
  doTouchMove = (e) => {
    this.props.onTouchMove(e, this.refs.Header);
  };
