...
import { updateInvoice } from "./../../../store/actions/financeAction";
...
class FinanceEditInvoice extends Component {
  constructor(props) {
    super(props);
    this.state = {
      // renderRow1
  ...
  componentDidMount() {
    ...
    const { invoiceData, allClients } = this.props.location.state;
    console.log(invoiceData);
    // set default edit invoice info
    this.setEditInvoiceDefaultData();
    // set client name dropdown option

    this.setDropdownOption();

    // set default items data
    this.setInvoiceDefaultItemsData();
  }

  setEditInvoiceDefaultData = () => {
    const { invoiceData, allClients } = this.props.location.state;
    let currentClient =
      !isEmpty(allClients) &&
    ...
    this.setState({
      ...
      clientEmail: currentClient[0].primaryContactPerson.email,
      billingAddress:
        currentClient[0].addresses.billing_line_one +
        "," +
        currentClient[0].addresses.shipping_line_one,
      fileName: isEmpty(invoiceData.documents)
        ? []
        : invoiceData.documents.map((doc) => doc.originalname),
      fileData: isEmpty(invoiceData.documents) ? [] : invoiceData.documents,
    });
  };

  setDropdownOption = () => {
    const { allClients, invoiceData } = this.props.location.state;
    ...
    if (!isEmpty(allClients)) {
      let dataIndex = newArray.findIndex(
        (client) => client.value === invoiceData.client
      );
      index = dataIndex;
    }
    ...
    this.setState({
      ...
      selectOption: newArray[index],
    });
  };

  setInvoiceDefaultItemsData = () => {
    const { invoiceData } = this.props.location.state;
    let finalItemArray = [];
    if (!isEmpty(invoiceData)) {
      invoiceData.items[0].map((item) => {
        finalItemArray.push({
          title: item.title,
  ...
  handleOnClickAdd = (status) => (e) => {
    ...
    const { billItems, fileData, invoiceData } = this.state;
    ...
    let previoudInvoiceData = invoiceData;
    previoudInvoiceData.invoice_number = this.state.invoiceNum;
    previoudInvoiceData.client = this.state.clientId;
    previoudInvoiceData.due_date = this.state.dueDate.toISOString();
    previoudInvoiceData.items = [finalItemArray];
    previoudInvoiceData.notes = this.state.additionalNotes;
    previoudInvoiceData.subTotal = subTotalAmount;
    previoudInvoiceData.totalTax = subTotalTaxAmount;
    previoudInvoiceData.total = totalAmount;
    previoudInvoiceData.documents = fileData;
    previoudInvoiceData.status = status;

    // console.log(previoudInvoiceData);
...
export default connect(null, { fileUpload, updateInvoice })(
  withRouter(FinanceEditInvoice)
);
